...
@interface NSScrollView (bowen)
@property(nonatomic) BOOL xc_reentrant;
@end

@implementation NSScrollView (bowen)

static char kNSSCrollViewReentrant;

- (void)setXc_reentrant:(BOOL)reentrant {
    objc_setAssociatedObject(self, &kNSSCrollViewReentrant, @(reentrant), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (BOOL)xc_reentrant {
    return [objc_getAssociatedObject(self, &kNSSCrollViewReentrant) boolValue];
}

- (void)xc_tile {
    BOOL isDVT = [NSStringFromClass([self class]) isEqual:@"DVTScrollView"];
    
    if (isDVT) {
        if (self.xc_reentrant)
            return;
        
        self.xc_reentrant = YES;
    }
    
    [self xc_tile];
    
    if (isDVT) {
        self.xc_reentrant = NO;
    }
}
@end
...
+ (void)load
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        error = nil;
        if (![NSScrollView jr_swizzleMethod:@selector(tile) withMethod:@selector(xc_tile) error:&error]) {
            NSLog(@"XcodeColors: Error swizzling NSScrollView methods: %@", error);
            return;
        }
